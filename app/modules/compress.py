"""
MÃ³dulo de compresiÃ³n de imÃ¡genes.
LÃ³gica pura de procesamiento, sin dependencias de UI.
"""
...
def _preparar_imagen(img: Image.Image, fmt: str) -> Image.Image:
    # Corregir rotaciÃ³n automÃ¡tica de cÃ¡maras y mÃ³viles
    img = ImageOps.exif_transpose(img)

    # CMYK â†’ RGB (modo impresiÃ³n no sirve en web/desktop)
    if img.mode == 'CMYK':
        img = img.convert('RGB')
    ...
    if fmt == 'PNG':
        # ReducciÃ³n a paleta de 256 colores â€” misma tÃ©cnica que TinyPNG
...
def comprimir_imagen(
    ...
    Si el resultado pesa mÃ¡s que el original, conserva el original.
    """
...
def estimar_tamano(ruta_entrada: str, calidad: int) -> int:
    ...
    with Image.open(ruta_entrada) as img:
        ...
        img.load()  # fuerza lectura del draft â€” sin reasignar
