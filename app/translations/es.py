"""
Traducciones al Español.
Diccionario de textos para la interfaz en idioma español.

Relacionado con:
    - app/translations/__init__.py: Sistema de traducciones.
    - app/translations/en.py: Traducciones en inglés (referencia).
    - app/translations/pt.py: Traducciones en portugués.

Este archivo contiene todas las claves de traducción usadas
en la aplicación organizadas por seccion:
    - App: Título general de la aplicación.
    - Sidebar: Labels de navegación.
    - Comprimir: Módulo de compresión.
    - Convert: Módulo de conversión.
    - Palette: Módulo de paleta de colores.
    - Metadata: Módulo de metadatos EXIF.
    - Resize: Módulo de redimensionado.
    - Settings: Módulo de configuración.
"""

TRANSLATIONS = {
    # App
    'app_title': 'Yagua - Procesador de Imágenes',
    'loading_model': 'Cargando modelo...',
    'downloading_model': 'Descargando modelo...',
    'analyzing_image': 'Analizando imagen...',
    'refining_edges': 'Refinando bordes...',
    'removing_bg': 'Quitando fondo...',
    'limit_reached': 'Se cargaron {limit} de {total} imágenes (límite máximo).',
    
    # Sidebar
    'compress': 'Comprimir',
    'convert': 'Convertir',
    'remove_bg': 'Quitar Fondo',
    'resize': 'Redimensionar',
    'rename': 'Renombrar Lote',
    'palette': 'Extraer Paleta',
    'watermark': 'Marca de Agua',
    'metadata': 'Metadatos EXIF',
    'lqip': 'LQIP / Base64',
    'ocr': 'Extraer Texto',
    'optimizer': 'Optimizador',
    'settings': 'Ajustes',
    'developed_by': 'Desarrollado por ',
    'github': '@guillebouix.dev',
    
    # Comprimir
    'compress_title': 'Comprimir',
    'select_images': 'Seleccionar imágenes',
    'no_images': 'Sin imágenes cargadas',
    'quality': 'Calidad',
    'remove_exif': 'Quitar EXIF',
    'compress_btn': 'Comprimir',
    'clean': 'Limpiar',
    'compressing': 'Comprimiendo...',
    'load_images_first': 'Carga al menos una imagen.',
    'select_output_folder': 'Elegí carpeta de salida',
    'estimated': 'estimado',
    'images_loaded': 'imágenes',
    'image_loaded': 'imagen',
    'compressed': 'más pequeño',
    'error_occurred': 'con error',
    'conflicts_renamed': 'conflictos renombrados',
    'size_reduction': 'reducción',
    
    # Convertir
    'convert_title': 'Convertir',
    'convert_to': 'Convertir a',
    'convert_btn': 'Convertir',
    'converting': 'Convirtiendo...',
    'converted_to': 'convertida a',
    'converted_to_plural': 'convertidas a',
    'load_images_first_convert': 'Carga al menos una imagen.',
    
    # Quitar Fondo
    'remove_bg_title':        'Quitar Fondo',
    'background_type':        'Fondo resultante',
    'bg_transparent':         'Transparente',
    'bg_white':               'Blanco',
    'bg_black':               'Negro',
    'bg_custom':              'Custom',
    'custom_color':           'Color',
    'pick_color':             'Elegir',
    'transparency_png_only':  'Transparente exporta como PNG.',
    'remove_bg_btn':          'Quitar fondo',
    'processing':             'Procesando...',
    'processed':              'procesadas',
    'rembg_unavailable_short':'No se pudo activar Quitar Fondo.',
    'rembg_missing_dependency':'Falta o está incompleta la dependencia: {dependency}.',
    'rembg_error_detail':     'Detalle técnico: {error}',
    'rembg_not_installed':    'rembg no está instalado.\nEjecutá este comando en tu terminal:',
    'rembg_dependency_error': 'No se pudo cargar rembg.\nError detectado: {error}',
    'rembg_reinstall_app':    'En algunos sistemas operativos o builds empaquetados, si ya instalaste la dependencia, puede hacer falta reconstruir y volver a instalar la app para reflejar el cambio.',
    'rembg_install_from_source': 'Si estás ejecutando el proyecto desde código fuente, podés instalar o reinstalar las dependencias con este comando:',
    'copy_command':           'Copiar comando',
    'command_copied':         'Comando copiado al portapapeles.',
    'model_first_download':   'El modelo de recorte se descargará automáticamente al procesar.',
    'output_format':         'Formato de salida',
    'remove_bg_description': 'Optimizado para productos y fondos claros: elimina el fondo y refina bordes automáticamente.',
    'remove_bg_fallback_notice': 'Usando modelo de compatibilidad ({model})',

    # Palette
    'palette_title': 'Paleta de colores',
    'colors_to_extract': 'Colores a extraer',
    'load_image_for_palette': 'Carga una imagen para ver la paleta',
    'save_palette': 'Guardar paleta como imagen',
    'save_palette_btn': 'Guardar Paleta de Colores',
    'no_image_selected': 'Sin imagen seleccionada',
    'select_image_for_palette': 'Selecciona una Imagen',
    'colors_extracted': 'colores extraídos',
    'click_format_to_copy': 'haz clic en un formato para copiar',
    'copied': 'Copiado:',
    'save_palette_first': 'Carga una imagen primero',
    'saved_as': 'Guardada como',
    'saving': 'Guardando...',
    'error_generic': 'Error',
    
    # Renombrar
    'rename_title':        'Renombrar Lote',
    'rename_numbering':    'Prefijo + Numeración',
    'rename_prefix':       'Prefijo',
    'rename_start':        'Inicio',
    'rename_replace':      'Reemplazar texto',
    'rename_search':       'Buscar',
    'rename_search_ph':       'texto a buscar',
    'rename_placeholder':     'foto',
    'rename_replace_with': 'Reemplazar por',
    'rename_replace_ph':   'texto nuevo',
    'rename_date':         'Agregar fecha',
    'rename_date_position':'Posición',
    'rename_prefix_pos':   'Prefijo',
    'rename_suffix_pos':   'Sufijo',
    'rename_date_format':  'Formato',
    'rename_case':         'Capitalización',
    'rename_case_none':    'Normal',
    'rename_case_lower':   'minúsc.',
    'rename_case_upper':   'MAYÚSC.',
    'rename_clean':        'Quitar espacios y caracteres especiales',
    'rename_preview':      'Preview',
    'rename_btn':          'Renombrar',
    'rename_col_original': 'Nombre original',
    'rename_col_nuevo':    'Nombre nuevo',
    'rename_preview_empty':'Cargá archivos para ver el preview',
    'files_loaded':        'archivos cargados',
    'file_loaded':         'archivo cargado',
    'load_files_first':    'Primero cargá archivos.',
    'renaming':            'Renombrando...',
    'renamed':             'renombrados',
    'rename_conflicts':    'conflictos omitidos',
    

    # Metadata
    'metadata_title': 'Metadatos EXIF',
    'view': 'Ver',
    'edit': 'Editar',
    'clean_batch': 'Limpiar lote',
    'select_image_view': 'Selecciona una imagen para ver sus metadatos',
    'select_image_edit': 'Selecciona una imagen para editar sus metadatos',
    'select_images_clean': 'Selecciona imágenes para limpiar EXIF',
    'no_metadata': 'Sin metadatos - carga una imagen',
    'no_metadata_image': 'Esta imagen no tiene metadatos EXIF',
    'view_on_maps': 'Ver en Maps',
    'export_txt': 'Exportar en .txt',
    'export_json': 'Exportar en .json',
    'export_metadata': 'Exportar metadatos',
    'reading_metadata': 'Leyendo metadatos...',
    'fields_found': 'campos encontrados',
    'export_metadata_first': 'Carga una imagen primero',
    'exported_as': 'Exportado como',
    'artist': 'Autor',
    'copyright': 'Copyright',
    'software': 'Software',
    'datetime': 'Fecha y hora',
    'enter_field': 'Ingresa {}...',
    'save_changes': 'Guardar cambios',
    'saving_changes': 'Guardando...',
    'saved_as_file': 'Guardado:',
    'saved_without_exif': 'guardado sin EXIF',
    'error_saving': 'Error al guardar',
    'enter_at_least_one': 'Ingresa al menos un campo para editar.',
    'editing': 'Editando:',
    'select_output_save': 'Guardar imagen editada',
    'images_ready_clean': 'imágenes listas para limpiar',
    'clean_exif': 'Limpiar EXIF',
    'cleaning': 'Limpiando...',
    'cleaned': 'imágenes limpiadas',
    'load_images_first_clean': 'Primero carga imágenes.',
    'select_output_folder_clean': 'Carpeta de salida',
    'without_exif': 'sinexif',
    
    # Resize
    'resize_title': 'Redimensionar',
    'resize_tab': 'Redimensionar',
    'crop_tab': 'Recortar',
    'canvas_tab': 'Canvas',
    'mode': 'Modo',
    'percentage': 'Porcentaje',
    'pixels': 'Píxeles',
    'preset': 'Preset',
    'maintain_aspect': 'Mantener relación de aspecto',
    'width': 'Ancho',
    'height': 'Alto',
    'size': 'Tamaño',
    'background': 'Fondo',
    'white': 'Blanco',
    'black': 'Negro',
    'transparent': 'Transparente',
    'apply_canvas': 'Aplicar Canvas',
    'processing': 'Procesando...',
    'images_loaded_resize': 'imágenes cargadas',
    'image_loaded_resize': 'imagen cargada',
    'processed': 'procesadas',
    'processed_singular': 'procesada',
    'load_images_first_resize': 'Carga una imagen primero',
    'invalid_dimensions': 'Ingresa ancho y alto válidos.',
    'warning_transparency': 'Advertencia: JPG/BMP no soportan transparencia - se usará el color negro.',
    'crop_centered': 'Recorte centrado - la imagen se recorta desde el centro.',
    'ratio': 'Ratio',
    'x_symbol': 'x',
    'redim': 'redim',
    'crop': 'crop',
    'canvas': 'canvas',

    # Image Transform
    'image_transform_title':    'Transformar Imagen',
    'transform_title':          'Transformar',
    'transform_quick_rotation': 'Rotación rápida',
    'transform_90_left':        '90° izquierda',
    'transform_180':            '180°',
    'transform_90_right':       '90° derecha',
    'transform_flip':           'Voltear',
    'transform_flip_h':         'Espejo horizontal',
    'transform_flip_v':         'Espejo vertical',
    'transform_free_rotation':  'Rotación libre',
    'transform_reset':          'Reset',
    'transform_exif':           'Corregir orientación EXIF (fotos de celular)',
    'transform_btn_apply':      'Aplicar transformaciones',
    'transform_select_one':     'Seleccioná al menos una transformación.',

    # LQIP
    'lqip_title':             'Low Quality Image Placeholders / Base64',
    'lqip_mode':              'Modo',
    'lqip_mode_lqip':         'LQIP',
    'lqip_mode_b64':          'Base64',
    'lqip_mode_lqip_desc':    'Genera una versión tiny y borrosa de la imagen.',
    'lqip_mode_b64_desc':     'Convierte la imagen completa a base64.',
    'lqip_width':             'Ancho (px)',
    'lqip_blur':              'Desenfoque',
    'lqip_quality':           'Calidad',
    'lqip_export':            'Exportar resultado',
    'lqip_export_field':      'Campo a exportar',
    'lqip_field_data_uri':    'Data URI',
    'lqip_field_html_tag':    'HTML Tag',
    'lqip_field_css_bg':      'CSS Background',
    'file_txt':               'Texto',
    'file_json':              'JSON',
    'lqip_actions':           'Acciones',
    'lqip_btn_process':       'Generar',
    'lqip_btn_copy':          'Copiar',
    'lqip_btn_save':          'Guardar',
    'lqip_ready_to_export':   'listo para exportar',
    'lqip_process_first':     'Primero generá el resultado.',
    'lqip_copied':            'Copiado al portapapeles.',
    'lqip_save_title':        'Guardar resultado',
    'lqip_saved_as':          'Guardado como',

    # OCR
    'ocr_title': 'Extraer Texto de Imágenes (OCR)',
    'select_images_ocr': 'Seleccionar Imágenes',
    'process_ocr': 'Procesar OCR',
    'processing_ocr': 'Procesando...',
    'ocr_processing': 'Procesando OCR...',
    'text_extracted': 'Texto extraído',
    'no_text_found': 'No se encontró texto en la imagen',
    'export_text': 'Exportar Texto',
    'export_as_txt': 'Exportar como .txt',
    'load_images_first_ocr': 'Carga al menos una imagen.',
    'ocr_model_loading': 'Cargando modelo OCR...',
    'ocr_unavailable': 'OCR no está disponible en este paquete.',
    'ocr_avif_requires_plugin': 'AVIF requiere pillow-avif-plugin',
    'heic_requires_plugin': 'HEIC requiere pillow-heif',
    'invalid_date_format': 'Formato de fecha inválido. Usa YYYY:MM:DD HH:MM:SS',
    'confirm_overwrite_title': 'Confirmar guardado',
    'confirm_overwrite_msg': '¿Sobrescribir el archivo original?',
    'datetime_format_placeholder': 'YYYY:MM:DD HH:MM:SS',
    'settings_tab': 'Ajustes',
    'updates_tab': 'Actualizaciones',
    'logs_tab': 'Logs',
    'updates_title': 'Actualizaciones',
    'current_version': 'Versión actual',
    'no_updates': 'No hay actualizaciones',
    'check_updates': 'Buscar actualizaciones',
    'updates_download_btn': 'Descargar actualización v{version}',
    'updates_manual_hint': 'La instalación es manual.',
    'home_hint': 'Selecciona un módulo en la barra lateral para comenzar.',
    'updates_checking': 'Buscando actualizaciones...',
    'updates_available': 'Nueva versión disponible: v{version}',
    'updates_downloading': 'Descargando actualización...',
    'updates_verifying': 'Verificando descarga...',
    'updates_installing': 'Instalando actualización...',
    'updates_requires_install': 'Auto-update solo disponible en la versión instalada.',
    'updates_hash_mismatch': 'Descarga inválida (verificación fallida).',
    'updates_error': 'No se pudo actualizar. Intentalo más tarde.',
    'logs_title': 'Logs',
    'log_date': 'Fecha',
    'log_level': 'Nivel',
    'log_module': 'Módulo',
    'log_message': 'Mensaje',
    'logs_empty': 'No hay logs para mostrar',
    'refresh': 'Actualizar',
    'ocr_batch_limit': 'Límite de lote: 10 imágenes',
    'ocr_description': 'Extrae texto de imágenes usando OCR.',
    'clear_text': 'Limpiar Texto',
    'copy_text': 'Copiar Texto',
    'text_copied': 'Texto copiado al portapapeles',
    'ocr_languages': 'Idiomas OCR',
    'ocr_lang_es_en': 'Español/Inglés',
    'ocr_lang_es': 'Español',
    'ocr_lang_en': 'Inglés',
    'select_language': 'Seleccionar idioma',

    # Vectorize
    "vectorizar":               "Vectorizar",
    "vectorizar_title":         "Vectorización de imágenes",
    "vectorizar_preset":        "Preset",
    "vectorizar_preset_photo": "Foto",
    "vectorizar_preset_illustration": "Ilustración",
    "vectorizar_preset_logo": "Logo",
    "vectorizar_preset_line_art": "Line Art",
    "vectorizar_preset_pixel_art": "Pixel Art",
    "vectorizar_preset_light_svg": "SVG Liviano",
    "vectorizar_preset_max_quality": "Máxima Calidad",
    "vectorizar_tipo":          "Tipo de vectorización",
    "vectorizar_color":         "Color",
    "vectorizar_byn":           "B&N",
    "vectorizar_detalle":       "Detalle",
    "vectorizar_detalle_bajo":  "Bajo",
    "vectorizar_detalle_alto":  "Alto",
    "vectorizar_limpieza":      "Limpieza",
    "vectorizar_limpieza_nada": "Nada",
    "vectorizar_limpieza_max":  "Agresivo",
    "vectorizar_suavidad":      "Suavidad",
    "vectorizar_suavidad_ang":  "Angular",
    "vectorizar_suavidad_sua":  "Suave",
    "vectorizar_tamano":        "Tamaño SVG",
    "vectorizar_tamano_cal":    "+ Calidad",
    "vectorizar_tamano_liv":    "+ Liviano",
    "vectorizar_btn_exportar":  "Exportar SVG",
    "vectorizar_exportadas":    "{ok} imagen(s) exportadas como SVG",
    "vectorizar_warning_distortion": "Aviso: la vectorización puede distorsionar la imagen original.",
    "vectorizar_omitidos_por_tamano": "{count} archivos omitidos. Límite de tamaño de 1 MB excedido.",
    "vectorizar_omitidos_por_formato": "{count} archivos omitidos. Formato no soportado para vectorizar.",

    # Watermark
    "watermark":                  "Marca de agua",
    "watermark_title":            "Marca de agua",
    "watermark_select_wm":        "Seleccionar imagen de marca de agua",
    "watermark_preset":           "Preset",
    "watermark_preset_subtle":    "Sutil",
    "watermark_preset_visible":   "Visible",
    "watermark_preset_protection":"Protección",
    "watermark_preset_center":    "Centro",
    "watermark_posicion":         "Posición",
    "watermark_pos_top_left":     "Arriba izq.",
    "watermark_pos_top_right":    "Arriba der.",
    "watermark_pos_bottom_left":  "Abajo izq.",
    "watermark_pos_bottom_right": "Abajo der.",
    "watermark_pos_center":       "Centro",
    "watermark_tamano":           "Tamaño",
    "watermark_tamano_pequeño":   "Pequeño",
    "watermark_tamano_grande":    "Grande",
    "watermark_opacidad":         "Opacidad",
    "watermark_opac_invisible":   "Invisible",
    "watermark_opac_solido":      "Sólido",
    "watermark_margen":           "Margen",
    "watermark_btn":              "Aplicar marca de agua",
    "watermark_procesadas":       "{ok} imagen(s) procesadas",
    "watermark_no_wm":            "Seleccioná una imagen de marca de agua primero",
    "watermark_preview_antes":    "ORIGINAL",
    "watermark_preview_despues":  "CON MARCA",
    "watermark_preview_placeholder": "Cargá una imagen para ver el preview",
    "watermark_calculando":       "Actualizando preview...",

    # Settings
    'settings_title': 'Ajustes',
    'language': 'Idioma',
    'select_language': 'Seleccionar idioma',
    'appearance': 'Apariencia',
    'theme': 'Tema',
    'ui_theme': 'Tema de la interfaz',
    'ui_scale': 'Escala de interfaz',
    'ui_scale_hint': 'Ajustá el tamaño global de la app para pantallas con demasiado zoom.',
    'dark': 'Oscuro',
    'light': 'Claro',
    'system': 'Sistema',
    'restart_required': 'Reiniciando aplicación para aplicar el idioma...',
    'visible_modules': 'Módulos visibles',
    'visible_modules_hint': 'Elegí los módulos que querés ver en la barra lateral.',
    'apply_changes': 'Aplicar',
    'restart_required_generic': 'Reiniciando aplicación para aplicar cambios...',
    'donate_paypal': 'Donar con PayPal',
    'donate_paypal_btn': 'Apoyar con PayPal',
    'donate': 'Apoyar el Proyecto',
}
